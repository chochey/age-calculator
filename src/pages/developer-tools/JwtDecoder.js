...
function JwtDecoder() {
  ...
  return (
    <div>
      <Seo title="JWT Decoder - Decode JSON Web Tokens Online" description="Free online JWT decoder. Paste a JSON Web Token to decode its header, payload, and check expiration. No data is sent to any server." faqs={[{ q: 'Is it safe to decode a JWT in the browser?', a: 'Yes. This decoder runs entirely on your device using client-side JavaScript. Your token never leaves the browser and is not transmitted to any server. That said, it is important to understand that JWTs are encoded, not encrypted. Anyone who obtains a JWT can decode the header and payload and read its contents. The signature ensures integrity (detecting tampering) but does not provide confidentiality. Avoid placing sensitive data like passwords or credit card numbers inside a JWT payload. If you need encrypted tokens, look into JWE (JSON Web Encryption).' }, { q: 'What do the exp, iat, and nbf claims mean?', a: 'These are registered time-based claims defined in the JWT specification (RFC 7519). iat (issued at) records the Unix timestamp when the token was created. exp (expiration time) is the Unix timestamp after which the token should be rejected. nbf (not before) is the earliest time at which the token should be accepted. All three values are expressed as seconds since January 1, 1970 (the Unix epoch). This decoder automatically converts exp and iat to human-readable dates and checks whether the current time has passed the expiration.' }, { q: 'What is the difference between HS256 and RS256 signing algorithms?', a: 'HS256 (HMAC with SHA-256) is a symmetric algorithm, meaning the same secret key is used to both sign and verify the token. It is simple to set up and works well when the issuer and verifier are the same application. RS256 (RSA Signature with SHA-256) is an asymmetric algorithm that uses a private key to sign and a separate public key to verify. RS256 is preferred in distributed systems where multiple services need to verify tokens without sharing a secret, because the public key can be freely distributed via a JWKS (JSON Web Key Set) endpoint.' }]} />
      <h1>JWT Decoder</h1>
