...
function TextRepeater() {
  ...
  return (
    <div>
      <Seo title="Text Repeater – QuickCalcs" description="Free online text repeater. Repeat any text multiple times with custom separators. Copy the result instantly." faqs={[{ q: 'Is there a limit to how many times I can repeat text?', a: 'The tool supports up to 10,000 repetitions per run. This limit ensures the browser remains responsive even with longer input text. For most practical purposes -- test data generation, content filling, or pattern creation -- 10,000 repetitions is more than sufficient.' }, { q: 'Can I use multi-line text as input?', a: 'Yes. The input field is a full text area, so you can paste paragraphs, code blocks, or any multi-line content. Each repetition will include the entire block of text exactly as you entered it, with your chosen separator placed between each copy.' }, { q: 'What happens when I choose a custom separator?', a: 'The custom separator option lets you type any string to place between each repetition. Common choices include pipe characters (|), dashes (-), semicolons (;), or even multi-character strings like " -- " or " | ". The separator is inserted between copies only, not before the first or after the last entry.' }]} />
      <h1>Text Repeater</h1>
