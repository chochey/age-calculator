...
function QrGenerator() {
  ...
  return (
    <div>
      <Seo title="QR Code Generator - Create QR Codes Free" description="Free online QR code generator. Create QR codes for URLs, text, email, phone numbers, and more. Download as PNG instantly." faqs={[{ q: 'What size should my QR code be?', a: 'For on-screen use (websites, emails, presentations), 200x200 to 300x300 pixels works well. For printed materials like business cards or flyers, use at least 300x300 pixels and ensure the printed size is no smaller than 2 cm on each side. For large-format printing like posters or banners, generate a larger code (400-500 pixels) and scale it up as needed -- QR codes remain scannable at any print size as long as the modules stay sharp.' }, { q: 'Do QR codes expire?', a: 'Static QR codes like those generated by this tool do not expire. The data is encoded directly in the image, so the code will work forever as long as the encoded content (such as a URL) remains active. If the linked website goes offline, the QR code will still scan but will lead to a dead page. Dynamic QR codes (which use a redirect service) can expire if the service is discontinued, but this generator produces static codes with no external dependencies.' }, { q: 'Can I customize the appearance of my QR code?', a: 'This generator produces standard black-and-white QR codes, which offer the highest scan reliability. If you need custom colors, embedded logos, or branded designs, you can use the downloaded PNG as a base and modify it in an image editor. Keep in mind that reducing contrast or covering too many modules with a logo can make the code unscannable, so always test the final design before distributing it.' }]} />
